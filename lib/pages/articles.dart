...
import 'package:the_india_insights/widgets/articleBoxFeatured.dart';
...
class _ArticlesState extends State<Articles> {
  ...
  List<dynamic> latestArticleslocal = [];
  ...
  Future<List<dynamic>?> loadListData(String key) async {
    ...
    if (jsonString != null) {
      ...
      print(decodedList);
      setState(() {
        latestArticleslocal = decodedList;
      });
      return decodedList;
    }
        ...
        if (response.statusCode == 200) {
          setState(() {
            ...
            latestArticles.addAll(json
                ...
                .toList());
            // String jsonString = jsonEncode(latestArticles);
            // print(jsonString);
            //
  ...
  Widget localData() {
    return ListView.builder(
      itemCount: latestArticleslocal.length,
      shrinkWrap: true,
      physics: NeverScrollableScrollPhysics(),
      itemBuilder: (context, index) {
        final heroId = latestArticleslocal[index]['id'].toString() + "-latest";
        return InkWell(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => SingleArticle(
                  Article.fromJson(latestArticleslocal[
    ...
    return FutureBuilder<List<dynamic>>(
      ...
      builder: (context, articleSnapshot) {
        ...
        if (articleSnapshot.hasData) {
          if (articleSnapshot.data!.length == 0) return Text("No Data");
          return Column(
            children: <Widget>[
              Column(
